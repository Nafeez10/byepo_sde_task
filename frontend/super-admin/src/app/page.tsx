...
import { OrganizationsAPI, useGetOrganizations } from '@/api/routes/OrganizationsAPI';
...
export default function Home() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const data = await OrganizationsAPI.createOrganization({ name, adminEmail });
    ...
    <div className="min-h-screen p-8 max-w-5xl mx-auto">
      <div className="flex justify-between items-center mb-12">
        <div>
          ...
          <p className="text-zinc-400 mt-1">Manage global organizations across the platform.</p>
        </div>
      ...
      <div className="grid md:grid-cols-[300px_1fr] gap-8">
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl h-fit sticky top-8">
          ...
          {inviteLink && (
            <div className="mt-6 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
              <h3 className="text-emerald-500 font-medium text-sm mb-2">Admin Invite Link Generated!</h3>
              <p className="text-zinc-400 text-xs mb-3">Share this link with the admin so they can set their password and log in.</p>
              <div className="flex gap-2">
                <input type="text" readOnly value={inviteLink} className="flex-1 bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-zinc-300 focus:outline-none" />
                <Button type="button" variant="outline" className="h-auto py-2" onClick={() => {
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-semibold text-white mb-6">Active Organizations</h2>
          {isLoading ? (
            <div className="text-zinc-500 animate-pulse">Loading organizations...</div>
          ) : orgs.length === 0 ? (
            ...
            <div className="grid gap-4">
              {orgs.map((org: any) => (
                <div key={org.id} className="bg-zinc-900/50 border border-zinc-800 p-5 rounded-2xl flex justify-between items-center hover:bg-zinc-900 transition-colors">
                  <div>
